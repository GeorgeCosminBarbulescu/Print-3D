...
jQuery(document).ready(function() {
    var NivoSlider = function(element, options){
        ...
        if(settings.controlNav){
			...
			// Add lava to nivo controll
			jQuery('<div class="lava" />').appendTo('.nivo-controlNav');
        ...
        var nivoRun = function(slider, kids, settings, nudge){          
            ...
            if(settings.controlNav)
			{
				...
				jQuery('.nivo-controlNav a').each(
					function(index)
					{
						var current = jQuery(this).attr('class');		
						var explode = current.split(' ');
						var elem_h  = jQuery(this).outerHeight(true);
						
						if( explode[1] == 'active' ) 
						{
							var rel = jQuery(this).attr('rel');	
							jQuery('.lava').animate({ top: (rel * elem_h) }, 600);
						}
					}
				); 
            }
   ...
   jQuery.fn._reverse = []._reverse||[].reverse;
